...
// ===== 追加: series.groupSkill を参照したグループトークン（例：ヌシの魂）判定 =====
const _variantKey = (v) => {
...
export const pickArmorCandidates = (
  selectedSkills,
  armorCatalog,
  { limitPerPart = 6, weight = { skill: 10, series: 6, group: 8, slot: 0.6 } } = {}
) => {
  ...
  const wantedGroupTokens = new Set(pickActiveGroupTokenRules(selectedSkills).map(r => norm(r.token)));
  ...
  const score = (p) => {
    ...
    // グループ（例：ヌシの魂）を欲している場合は該当variantをブースト
    if (wantedGroupTokens.size) {
      const rec = sKey ? groupIndex.get(sKey) : null;
      const tok = rec ? rec[_variantKey(p?.variant)] : null;
      if (tok && wantedGroupTokens.has(tok)) sc += (weight.group || 8);
    }
    return sc;
  };
